...
KeyboardGetter = Callable[[FSMContext], Awaitable[Optional[ReplyKeyboardMarkup]]]
...
def produce_simple_get_stage(
    ...
    question_regular_kb_getter: KeyboardGetter = get_empty_reply_markup,
    question_inline_kb_getter: KeyboardGetter = Callable[[FSMContext], Awaitable[Optional[ReplyKeyboardMarkup]]] get_empty_reply_markup,
) -> StageType:
    class SimpleGetStage(Stage):
        state = State(state=stage_name)
        name: str = stage_name
        question_regular_kb: KeyboardGetter
